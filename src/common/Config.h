...
public:
  ...
  // sjudge 子进程安全设置
  uint32_t getSjudgeRunUid() const;
  uint32_t getSjudgeRunGid() const;
  bool getSjudgeClearSupplementaryGroups() const;
  bool getSjudgeEnableSeccomp() const;
  bool getSjudgeSeccompDenyNetwork() const;
  bool getSjudgeSeccompDenyProcessSpawn() const;
  std::string getSjudgeCgroupPath() const;
  uint64_t getSjudgeCgroupMemoryMaxBytes() const;
  uint32_t getSjudgeCgroupPidsMax() const;
  uint64_t getSjudgeCgroupCpuMaxQuotaUs() const;
  uint64_t getSjudgeCgroupCpuMaxPeriodUs() const;
...
private:
  ...
  mutable std::mutex
      config_mutex_; // 保护对config_data_的访问，如果运行时可以修改的话
  ...
  template <typename T>
  T get(const json &subObject, const std::string &key,
...
T Config::get(const std::string &key, const T &defaultValue) const {
  std::lock_guard<std::mutex> lock(
      config_mutex_); // 如果配置可能改变，确保线程安全读取
  try {
...
// 从特定JSON子对象获取值的辅助函数
template <typename T>
T Config::get(const json &subObject, const std::string &key,
              const T &defaultValue) const {
  ...
  try {
    ...
    std::cerr << "子对象配置访问错误，键 '" << key << "': " << e.what()
              << std::endl;
  }
  return defaultValue;
}
