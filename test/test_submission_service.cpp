...
void test_submit_creates_submission_and_persists_result_snapshot() {
    ResultStore store;
    RunnerFactory factory;
    JudgeCore judge_core;
    SubmissionService service = make_service(store, factory, judge_core);

    const int submission_id = service.submit(make_python_request(
...
void test_successful_submission_finishes_with_aggregated_ac_verdict() {
    ResultStore store;
    RunnerFactory factory;
    JudgeCore judge_core;
    SubmissionService service = make_service(store, factory, judge_core);
...
void test_compile_failure_finishes_with_ce() {
    ...
    const int submission_id =
        service.submit(make_python_request(93003, "def broken(:\n    pass\n"));
...
void test_unsupported_language_finishes_as_deterministic_system_failure() {
    ...
    const int submission_id = service.submit(make_unsupported_request(93004));
...
int main() {
    test_submit_creates_submission_and_persists_result_snapshot();
    test_successful_submission_finishes_with_aggregated_ac_verdict();
