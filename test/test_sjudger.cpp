...
void test_cgroup_missing_directory_reports_system_error() {
  judge_config config{};
  config.exe_path = "/bin/true";
  config.args = {"/bin/true"};
  config.cgroup_path = "/tmp/boxTest_missing_cgroup_for_test";

  const judge_result result = run_sjudger(config);

  assert(result.result == SYSTEM_ERROR);
  assert(result.error == CGROUP_FAILED);
}

#ifdef SJUDGER_ENABLE_SECCOMP
void test_seccomp_can_reject_network_socket() {
  const fs::path dir = make_sjudger_dir("sjudger_seccomp_network");
  const fs::path script = dir / "network.py";
  const fs::path output = dir / "out.txt";
...
int main() {
  ...
  test_cgroup_missing_directory_reports_system_error();
#ifdef SJUDGER_ENABLE_SECCOMP
  test_seccomp_can_reject_network_socket();
#endif
  return 0;
}
